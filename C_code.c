...
#define PROC_IS_ROOT(aProc) ((uintptr_t)aProc <= (u32)PROC_TREE_7)
...
#endif
    {
        ...
        proc =
            (PROC_IS_ROOT(parent)) ? Proc_Start(ProcScr_GotItem, parent) : Proc_StartBlocking(ProcScr_GotItem, parent);
...
#define MinPromotedStat 2
    ...
    int GetAutoleveledStatDecrease(int growth, int levelCount, int stat)
    {
        ...
        int result = stat -
            NewGetStatDecrease(
                         (growth * levelCount) + (NextRN_N((growth * levelCount) / 4) - (growth * levelCount) / 8));
...
#define UseRandClass2
        ...
        noise[0] = character->number + character->baseLevel + character->baseHP + character->basePow +
            character->baseSkl + character->baseSpd + character->baseDef + character->baseRes + character->baseLck;
        noise[1] = unit->index + character->portraitId + character->growthHP + character->growthPow +
            character->growthSkl + character->growthSpd + character->growthDef + character->growthRes +
            character->growthLck;
                ...
                { // 80%, as HashByte never returns the max number
                    ...
                    if (prepromoteClassId)
                    {
                        ...
                        { // some classes (eg. Manakete, King) "promote" from ballista
                            ...
                            unit->pClassData =
                                originalClass; // so RandClass will treat us as promoted or not based on that
#ifdef UseRandClass2
        ...
        if (RandomizeMovConBonus)
        {
            if (RandBitflags->base)
            {
                if (IsUnitAlliedOrPlayable(unit))
                {
                    ...
                    {
#ifdef FE6
                        unit->movBonusA = MovModifiers[HashByte_Global(
                            3, sizeof(MovModifiers), noise, 14)]; // num, max, noise, offset
                        unit->movBonusB = unit->movBonusA;
#endif
#ifndef FE6
                        unit->movBonus = MovModifiers[HashByte_Global(
                            3, sizeof(MovModifiers), noise, 14)]; // num, max, noise, offset
#endif
                    }
                }
        ...
        if (RandomizeRecruitment)
        {
            ...
            bonusLevels +=
                GetAdjustedLevel(character, originalClass) - GetAdjustedLevel(character, randCharOriginalClass);
            if (bonusLevels)
            {
...
#ifdef FE6
            PutDrawText(
                gStatScreen.text + 9, gUiTmScratchA + TILEMAP_INDEX(9, 1), gold, 0, 0, GetStringFromIndex(0xB80));
#endif
        }
        else
        {
        ...
        if (gStatScreenFunction[id].specialCase)
        {
            switch (gStatScreenFunction[id].specialCase)
            {
                case 1:
                {
                    ...
                    { // 9, 1?
                        PutDrawText(
                            gStatScreen.text + STATSCREEN_TEXT_MOVLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold,
                            0, 0, "Mov");
                        DrawStatWithBar(
                ...
                {
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_CONLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Con");
                    DrawStatWithBar(
                        barID, x, y, gStatScreenFunction[id].GetUnitUnadjustedStat(unit),
                        gStatScreenFunction[id].GetUnitStat(unit), gStatScreenFunction[id].GetUnitMaxStat(unit));
                ...
                case 3:
                {
                    ...
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_AIDLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Aid");
                ...
                {
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_AFFINLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Affin");
                ...
                case 7:
                {
                    ...
                    {
                        PutDrawText(
                            gStatScreen.text + STATSCREEN_TEXT_ITEM0, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                            0, "Talk");
                        talk_uid = GetCharacterData(talk_uid)->nameTextId;
                        Text_InsertDrawString(
                    ...
                    {
                        PutDrawText(
                            gStatScreen.text + STATSCREEN_TEXT_RESCUENAME, gUiTmScratchA + TILEMAP_INDEX(x - 4, y),
                            gold, 0, 0, "Trv");
                        Text_InsertDrawString(
                            &gStatScreen.text[STATSCREEN_TEXT_RESCUENAME], 24, blue, GetUnitRescueName(unit));
                    }
                ...
                { // skills row 2
                    ...
                    for (int i = 0; i < 6; ++i)
                    {
                        ...
                        if (i > 2)
                        {
                            ...
                            {
                                DrawIcon(
                                    gUiTmScratchA + TILEMAP_INDEX(x - 4 + ((i - 3) * 3), y), *skillID | 0x100, 0x4000);
                            }
                            else
                            {
                ...
                case 11:
                {
                    if (!StrMagInstalled)
                    {
                        if (UnitHasMagicRank(gStatScreen.unit))
                        {
                            ...
                            PutDrawText(
                                &gStatScreen.text[STATSCREEN_TEXT_POWLABEL], gUiTmScratchA + TILEMAP_INDEX(x - 4, y),
                                gold, 0, 0, "Pow"); // Pow / Mgc / Atk / Mag
                                                    // #endif
                        }
                        else
                        {
// str
                            ...
                            PutDrawText(
                                &gStatScreen.text[STATSCREEN_TEXT_POWLABEL], gUiTmScratchA + TILEMAP_INDEX(x - 4, y),
                                gold, 0, 0, GetStringFromIndex(0x4FE)); // Str
#else
                            PutDrawText(
                                &gStatScreen.text[STATSCREEN_TEXT_POWLABEL], gUiTmScratchA + TILEMAP_INDEX(x - 4, y),
                                gold, 0, 0, "Str"); // Str
                ...
                case 12:
                {
                    ...
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_SKLLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Skl");
                    // #endif
                    break;
                }
...
#ifdef FE8
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_SPDLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, GetStringFromIndex(0x4ED));
#else
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_SPDLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Spd");
                ...
                case 14:
                {
                    ...
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_LCKLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Lck");
                    // #endif
                    break;
                }
...
#ifdef FE8
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_DEFLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, GetStringFromIndex(0x4EF));
#else
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_DEFLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Def");
...
#ifdef FE8
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_RESLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, GetStringFromIndex(0x4F0));
#else
                    PutDrawText(
                        gStatScreen.text + STATSCREEN_TEXT_RESLABEL, gUiTmScratchA + TILEMAP_INDEX(x - 4, y), gold, 0,
                        0, "Res");
