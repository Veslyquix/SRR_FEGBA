...
# I like to see how many hash collisions happen.
HASHCOLLISIONS = 0
...
class SpriteChunk:
  def __init__(self, image, x, y, w, h, palID):
    ...
    if w == h:
      shape = 0
      # Upstream bug fix: unlike the w>h/h>w branches below, this never special-cased
      # the maximum (64x64, w==8 tiles) case - w>>1 gives 4, which doesn't fit the
      # OAM size field's 2 bits (0-3) and crashes to_bytes() with an OverflowError.
      # GBA's square OAM size codes are 0/1/2/3 for 8x8/16x16/32x32/64x64.
...
# Main parse script.
# Returns outputFile's (the EA Installer) name.
def main():
  ...
  parser.add_argument('-a', '--address', nargs='?', const='0', help='Optional hexadecimal address in ROM to insert banim entry into (the struct containing the pointers to the data, not the data itself). If not set, user will need to manually open output installer file and set AnimEntry ID of the banim to take in ROM.')
  ...
  if args.address is not None:
    ...
    # Sanity check, user may have left in an 0x800 0000.
    address &= 0xF7FFFFFF
    
    # Advance past first 8 bytes of name string.
    address += 8
  ...
  head, animName = os.path.split(args.filename[:-4])
  outputFileName = animName+"Installer.event"
  # Explicit UTF-8 (with a lossless-for-ASCII fallback): a few of this repo's scripts
  # carry UTF-8 comments (e.g. a Japanese credit), and opening without an encoding
  # defaults to the OS locale (cp1252 on Windows), which crashes on those bytes.
  scriptFile = open(args.filename, "r", encoding="utf-8", errors="replace")
  ...
  for i, line in enumerate(scriptFile):
    if header and line[0] == '@':           # Extra setting.
      ...
      if line[2:15] == "UNCOMPPALDATA":
        ...
        print("ERROR: line: " + str(i+1) + ", Couldn't recognize @ code: " + line[2:-1] + ". README explains which @ codes are recognized.")
        sys.exit()
    else:
      header = False;
    ...
    if line[0] == '~':          # Mode end.
      ...
      if (len(imageFileName) <= 0) or (not os.path.isfile(imageFileName)):
        print("ERROR: line: " + str(i+1) + ", image file: " + imageFileName + " not found.")
        print("CWD: " + os.getcwd())
        sys.exit()
  ...
  #define AnimTableEntry(index) \"ORG ClassAnimTable + 8 + ((index) * 0x20)\"
#endif\n\n"""
  animEntry = """{{\nPUSH\n"""
  
  if (address == 0):
    animEntry += """  AnimTableEntry({animName}) // CHANGE THIS TO THE SLOT YOU ARE REPLACING"""
  else:
    animEntry += """  ORG """ + hex(address)
  
  # Upstream bug fix: this table's real stride is 0x20 bytes (see SRR's own
  # AnimTableEntry macro, Internals.event - "(index) * 0x20"), which
  # Internals.event's own DefaultAnimEntry confirms is a 12-byte name field
  # followed by the 5 pointers (12 + 5*4 = 32). AA.exe's own output always
  # padded this field to 3 words ("WORD 0 0 0 //empty name field, who
  # cares"). Upstream only ever wrote ONE word here - 8 bytes short of the
  # stride - which shifts every pointer 8 bytes early and leaves the last
  # 8 bytes of each entry (part of the ltr/pal pointers) never written.
  animEntry +="""\n  WORD {settingBitfield} 0 0
...
POP"""
  ...
  # Write defs to help user set AnimEntry ID if no address provided.
  if (address == 0):
