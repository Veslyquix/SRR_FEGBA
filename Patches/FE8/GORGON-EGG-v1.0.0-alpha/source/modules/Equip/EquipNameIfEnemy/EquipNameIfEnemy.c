...
struct EquipNameIfEnemyProc
{
  /* 00 */ PROC_FIELDS;
  /* 2C */ struct TextHandle equipText;
};

const char EquipNameIfEnemyProc_Name[] = "EquipNameIfEnemy";

const struct ProcInstruction ProcEquipNameIfEnemy[] = {
  PROC_SET_NAME(&EquipNameIfEnemyProc_Name),
  PROC_SLEEP(0),
...
void EquipNameIfEnemy_Static(struct PlayerInterfaceProc* proc, struct UnitDataProc* udp)
{
  /* Draws an enemy unit's equipped weapon's name.
   */

  unsigned padding;
  char* equipString;

  struct EquipNameIfEnemyProc* equipProc = (struct EquipNameIfEnemyProc*)ProcFind(ProcEquipNameIfEnemy);
  if ( equipProc == NULL )
  {
    equipProc = (struct EquipNameIfEnemyProc*)START_PROC(ProcEquipNameIfEnemy, proc);
    Text_InitDB(&equipProc->equipText, EQUIP_NAME_IF_ENEMY_WIDTH);
  }

  Text_Clear(&equipProc->equipText);

  if ( (UNIT_FACTION(udp->unit) == FACTION_RED) && (udp->weapon != 0) )
  {
    ...
    if ( EQUIP_NAME_IF_ENEMY_ALIGNMENT == EQUIP_NAME_CENTERED )
      padding = Text_GetStringTextCenteredPos((EQUIP_NAME_IF_ENEMY_WIDTH * 8), equipString);

    else if ( EQUIP_NAME_IF_ENEMY_ALIGNMENT == EQUIP_NAME_LEFT_ALIGNED )
      padding = 0;

    else if ( EQUIP_NAME_IF_ENEMY_ALIGNMENT == EQUIP_NAME_RIGHT_ALIGNED )
      padding = (EQUIP_NAME_IF_ENEMY_WIDTH * 8) - Text_GetStringTextWidth(equipString);

    else
      padding = Text_GetStringTextCenteredPos((EQUIP_NAME_IF_ENEMY_WIDTH * 8), equipString);

    Text_SetParameters(&equipProc->equipText, padding, EQUIP_NAME_IF_ENEMY_COLOR);
    Text_DrawString(&equipProc->equipText, equipString);

    Text_Display(&equipProc->equipText, gUiTmScratchA + TILEMAP_INDEX(EQUIP_NAME_IF_ENEMY_X, EQUIP_NAME_IF_ENEMY_Y));
  }
