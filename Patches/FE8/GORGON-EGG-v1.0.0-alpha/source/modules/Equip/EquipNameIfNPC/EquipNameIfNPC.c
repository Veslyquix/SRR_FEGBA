...
struct EquipNameIfNPCProc
{
  /* 00 */ PROC_FIELDS;
  /* 2C */ struct TextHandle equipText;
};

const char EquipNameIfNPCProc_Name[] = "EquipNameIfNPC";

const struct ProcInstruction ProcEquipNameIfNPC[] = {
  PROC_SET_NAME(&EquipNameIfNPCProc_Name),
  PROC_SLEEP(0),
...
void EquipNameIfNPC_Static(struct PlayerInterfaceProc* proc, struct UnitDataProc* udp)
{
  ...
  struct EquipNameIfNPCProc* equipProc = (struct EquipNameIfNPCProc*)ProcFind(ProcEquipNameIfNPC);
  if ( equipProc == NULL )
  {
    equipProc = (struct EquipNameIfNPCProc*)START_PROC(ProcEquipNameIfNPC, proc);
    Text_InitDB(&equipProc->equipText, EQUIP_NAME_IF_NPC_WIDTH);
  }

  Text_Clear(&equipProc->equipText);

  if ( (UNIT_FACTION(udp->unit) == FACTION_GREEN) && (udp->weapon != 0) )
  {
    ...
    if ( EQUIP_NAME_IF_NPC_ALIGNMENT == EQUIP_NAME_CENTERED )
      padding = Text_GetStringTextCenteredPos((EQUIP_NAME_IF_NPC_WIDTH * 8), equipString);

    else if ( EQUIP_NAME_IF_NPC_ALIGNMENT == EQUIP_NAME_LEFT_ALIGNED )
      padding = 0;

    else if ( EQUIP_NAME_IF_NPC_ALIGNMENT == EQUIP_NAME_RIGHT_ALIGNED )
      padding = (EQUIP_NAME_IF_NPC_WIDTH * 8) - Text_GetStringTextWidth(equipString);

    else
      padding = Text_GetStringTextCenteredPos((EQUIP_NAME_IF_NPC_WIDTH * 8), equipString);

    Text_SetParameters(&equipProc->equipText, padding, EQUIP_NAME_IF_NPC_COLOR);
    Text_DrawString(&equipProc->equipText, equipString);

    Text_Display(&equipProc->equipText, gUiTmScratchA + TILEMAP_INDEX(EQUIP_NAME_IF_NPC_X, EQUIP_NAME_IF_NPC_Y));
  }
