...
# --- Team-colour (Enemy/NPC/Other) palette auto-detection ------------------
#
# The battle-animation palette has 4 rows of 16 colours: player, enemy, NPC,
# and "other" (grey/3rd army). AA.exe reads these from the proprietary
# FEditor Adv .bin project file, which can bundle hand-picked colours for
# each row that don't exist anywhere else. Some FE-Repo packs *also* ship
# that data openly, as a sibling folder (name containing "palette") holding
# one reference image per team, each already indexed against that team's
# 16-colour palette (e.g. "team_palettes/Palette 02_enemy.png"). When such a
# folder exists we use it, matching AA.exe byte-for-byte. When it doesn't,
# there is no way to recover the artist's exact team colours from the open
# PNG/txt export, so we duplicate the player row into the missing slots
# rather than emit black (0x0000) rows, which would render as solid-black
# sprites for those factions in-game.
#
# NB: a loose, ambiguously-named image sitting directly in the pack root
# (e.g. some packs ship a "NPC.png" preview that is *not* the palette
# source -- verified against a real AA.exe output where it didn't match at
# all) is deliberately NOT matched. Only files inside a dedicated
# "*palette*"-named folder are trusted.
TEAM_PALETTE_KEYWORDS = {
  'enemy': ('enem',),
  'npc': ('npc',),
  'other': ('other', 'grey', 'gray'),
}

def rgbListToGbaPaletteBytes(rgbFlatList, count=16):
  pal = [c >> 3 for c in rgbFlatList[:count*3]]
  words = [pal[i] | (pal[i+1]<<5) | (pal[i+2]<<10) for i in range(0, len(pal), 3)]
  return b''.join(w.to_bytes(2, byteorder='little') for w in words)

def firstColorsFromImage(imagePath, count=16):
  ...
  if not pal:
    return None
  return rgbListToGbaPaletteBytes(pal, count)

def findTeamPaletteImages(scriptDir):
  parent = os.path.dirname(scriptDir)
  if not os.path.isdir(parent):
    return {}
  paletteDirs = [os.path.join(parent, e) for e in os.listdir(parent)
                 if 'palette' in e.lower() and os.path.isdir(os.path.join(parent, e))]
  found = {}
  for d in paletteDirs:
    for entry in sorted(os.listdir(d)):
      if not entry.lower().endswith('.png'):
        continue
      lower = entry.lower()
      for team, keywords in TEAM_PALETTE_KEYWORDS.items():
        if team in found:
          continue
        if any(kw in lower for kw in keywords):
          found[team] = os.path.join(d, entry)
  return found
...
POP"""
  ...
  if palsize == 128:
    # Standard 4-row (player/enemy/npc/other) battle-anim palette. Fill
    # rows 1-3 from an adjacent team-palette folder when the pack ships
    # one; otherwise fall back to the player row so unmatched teams still
    # render in-colour instead of solid black.
    row0 = rgbListToGbaPaletteBytes(frame.palette, 16)
    teamImages = findTeamPaletteImages(os.path.dirname(os.path.abspath(args.filename)))
    rows = [row0]
    for team in ('enemy', 'npc', 'other'):
      imgPath = teamImages.get(team)
      rows.append(firstColorsFromImage(imgPath, 16) if imgPath else row0)
    pal = b''.join(rows)
  else:
